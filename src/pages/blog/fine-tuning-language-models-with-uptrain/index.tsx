  ...
  return (
    ...
    >
      <div>
        ...
        <p>
          The era of large language models (LLMs) taking the world by storm has
          come and gone. Today, the debate between proponents of bigger models
          and smaller models has intensified. While the debate continues, one
          thing is clear: not everyone needs to run large models for their
          specific use-cases. In such situations, {"it's "}
        ...
        <p>
          In this blog post, {"we'll"} walk you through a tutorial to collect a
          dataset to fine-tune a model that can summarize human conversations.
          The model {"we're"} working with is based on the
        ...
        <p>
          ...
          conversations and their summaries. To do this, {"we'll"} use UpTrain,
          a powerful tool for creating fine-tuning datasets.
        </p>
        <h2 id="the-process">The Process</h2>
        <p>
          In this blog, the process of collecting a fine-tuning dataset involves
          several steps:
        </p>
        <p>● Visualizing UMAP/t-SNE for low-performing clusters</p>
        <p>● Finding clusters around data-points where accuracy is low</p>
        <p>
          ● Edge-case Collection (user defines the edge-case parameters based on
        ...
        <p>
          ...
          SAMSum and DialogSUM datasets. Due to this, running this entire script
          does not take too much time (e.g., it runs in 3 minutes on my Macbook
          Air).
        </p>
        <p>
          First, {"let's"} see (literally) what we are dealing with. We plot the
          sentence BERT embeddings with UMAP dimensionality reduction. We apply
          dimensionality reduction on 3 types of datasets: SAMSum train (aka
          reference dataset), SAMSum test, and DialogSum train.
        </p>
        ...
        <p>
          ...
          outliers because they are more likely to perform worse (and as we show
          later, they do!).
        </p>
        <p>
          To do this, {"we'll"} use a technique called sentence BERT embeddings
          to represent our text data. {"We'll"} compare these embeddings from
          the DialogSum dataset to the ones from our reference dataset (SAMSum
          training).
        </p>
        <p>
          <b>RESULT</b>: The overall performance (Rogue-L score) on the
          DialogSum dataset was 0.305. However, on the data-points identified by
          the method above, the performance score dropped to 0.237.
        </p>
        <p>
          While analyzing the model outputs above, we made a few observations on
          cases where model does not perform well. Note that these are not
        ...
        <p>
          ...
          appropriate edge-cases. We do this by making a few observations on the
          model performance.
        </p>
        <p>
          <b>Observation</b>: Our first observation is that the model has
          trouble summarizing very long conversations, often producing
          incomplete summaries. Some examples of these incomplete summaries are
          below.
        </p>
        <CodeViewer>
    ...
    "Jen wants to break up with her boyfriend. He hasn't paid her back the"`}
        ...
        <p>
          ...
          collect a dataset that can help the model improve in these edge cases,
          we need to find conversations that are longer than a certain
          threshold. To find an appropriate threshold, we generated a histogram
          of length of input dialogues on the training dataset (i.e., SAMSum
          train). From here, we noted that a length of 1700 can be a good
          cut-off to collect large conversation data-points. The histogram is
          shown below.
        </p>
        <Image
          style={{ height: "auto" }}
        ...
        <p>
          Following is how the edge-case check is defined in UpTrain. It checks
          if the length of the input dialogue is greater than 1700 characters.
        </p>
        <CodeViewer>
    ...
    edge_case_length = {
        ...
        <p>
          ...
          that affects the performance of our language model. In this case, the
          model directly copies one or two sentences from the input
          conversation. This works well in many cases but fails when the
          conversation is about refuting those one or two sentences which model
          produced as summary. This can lead to the generation of summaries that
          are not accurate or do not capture the true essence of the
          conversation.
        </p>
        <p>
          For example, the output summary of the model in the following cases is
          not appropriate:
        </p>
        <CodeViewer>{`Input:
    ...
    Output: Person1 is looking for a shop that sells inexpensive cashmere sweaters.`}</CodeViewer>
        ...
        <p>
          In order to catch this heuristic as an edge case in UpTrain, we define
          two functions: rogueL_check_func and negation_func.
        </p>
        <p>
          The rogueL_check_func function checks whether sentences from the input
          are copied directly using the Rouge-L metric. This metric calculates
          the longest common subsequence of characters in the input and output
          texts.
        </p>
        <p>
        ...
        <p>
          By identifying and addressing these edge cases, we can further improve
          the performance of our language model, making it more accurate and
          reliable in generating summaries of conversations.
        </p>
        ...
        <p>
          ...
          determine whether there is a significant shift in the vocabulary used
          between the two datasets. This will help us understand how well the
          model can generalize and produce accurate summaries for different
          datasets.
        </p>
        <p>
        ...
        <p>
          ● vocab_init: Initializes the state of the monitor, which includes the
          training vocabulary and a counter for out-of-vocab words.
        </p>
        <p>
        ...
        <p>
          ...
          of our model and identify any vocabulary drift that might be affecting
          the {"model's"} performance. This allows us to make the necessary
          adjustments to improve the accuracy and reliability of our language
          model in generating summaries for various datasets.
        </p>
        <p>
          <b>Defining UpTrain Config and Framework</b>
        </p>
        <p>
          In this section, we set up the UpTrain configuration and framework to
          analyze our {"model's"} performance and collect the necessary data.
          This helps us ensure that our language model is generating accurate
          summaries for various datasets.
        </p>
        <p>
          We create a configuration dictionary that includes the edge cases and
          custom monitor we defined earlier. Additionally, we specify the folder
          where the smart data will be stored.
        </p>
        <CodeViewer>
          {`config = {
    ...
    framework = uptrain.Framework(cfg_dict=config)`}
        ...
        <p>
          Using the UpTrain dashboard, we can visualize the vocabulary coverage
          of our model on the production data. The coverage starts at around 98%
          for the SAMSum test dataset but decreases to about 95% for the
          DialogSum dataset (that is, after ~800 SAMSum test points are logged).
        </p>
        <Image
          style={{ height: "auto" }}
        ...
        <p>
          By understanding the out-of-vocabulary words and their context, we can
          take steps to improve our {"model's"} performance on specific topics
          or regions, ensuring it generates accurate and relevant summaries for
          various datasets, as shown next.
        </p>
        ...
        <p>
          ...
          SAMSum dataset. So, {"we'll"} create a check to catch cases containing
          specific Asian words. The list asian_words contains some common Asian
          words like names, places, and currencies, and the check goes on to
          define cases where Asia-related words are found in the input dialogue.
        </p>
        <CodeViewer>
    ...
    edge_case_asian_word = {
        ...
        <p>
          ...
          custom use-cases like summarizing long conversations in the DialogSum
          dataset.
        </p>
        <p>
          pIn summary, fine-tuning smaller language models using tools like
          UpTrain is an effective way to create tailored solutions for specific
          tasks. By identifying edge cases, building custom monitors, and
          analyzing the {"model's"} performance, we can collect high-quality
          datasets that help improve the {"model's"} performance for our
          particular use-case.
        </p>
      </div>
    </BlogTemplate>
