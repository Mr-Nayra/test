        ...
        >
          <div>
            ...
            <p>
              ...
              more practical to collect high-quality datasets to fine-tune
              smaller models for the task at hand.
            </p>
            <h2 id="enhancing-a-conversation-cummarization-codel-with-uptrain">
              Enhancing a Conversation Summarization Model with UpTrain
            </h2>
            <p>
              In this blog post, {"we'll"} walk you through a tutorial to
              collect a dataset to fine-tune a model that can summarize human
            ...
            <p>
              Our goal is to further improve this model so that it performs
              better on a similar dataset called DialogSum, which also contains
            ...
            <p>
              ● Edge-case Collection (user defines the edge-case parameters
              based on heuristics/observations)
            </p>
            <p>
            ...
            <p>
              We understand that running the bart-large-xsum can be time
              consuming on some machines. Hence, we have pre-generated the model
              outputs and their corresponding sentence BERT embeddings to remote
              for both the SAMSum and DialogSUM datasets. Due to this, running
              this entire script does not take too much time (e.g., it runs in 3
              minutes on my Macbook Air).
            </p>
            <p>
              First, {"let's"} see (literally) what we are dealing with. We plot
              the sentence BERT embeddings with UMAP dimensionality reduction.
              We apply dimensionality reduction on 3 types of datasets: SAMSum
              train (aka reference dataset), SAMSum test, and DialogSum train.
            </p>
            <h2 id="Analyzing-Performance-and-Visualizing-with-UMAP">
              Analyzing Performance and Visualizing with UMAP
            </h2>
            <p>
              {"We'll"} also use a visualization technique called UMAP to see
              how different datasets are related in terms of content. Datasets
              marked reference (i.e., SAMSum training) and samsum (i.e., SAMSum
              test) are close in the UMAP space. Most point from the DialogSum
              dataset are further than the data on which the model was finetuned
              on (i.e., reference aka SAMSum train).
            </p>
            <Image
              style={{ height: "auto" }}
              src={analyzing}
            ...
            />
            <p>
              Next, we identify poorly performing points and find clusters
              around them.
            </p>
            ...
            <p>
              ...
              outliers. Our objective is to find data-points that lie around
              these outliers because they are more likely to perform worse (and
              as we show later, they do!).
            </p>
            <p>
              To do this, {"we'll"} use a technique called sentence BERT
              embeddings to represent our text data. {"We'll"} compare these
              embeddings from the DialogSum dataset to the ones from our
              reference dataset (SAMSum training).
            </p>
            <p>
              <b>RESULT</b>: The overall performance (Rogue-L score) on the
              DialogSum dataset was 0.305. However, on the data-points
              identified by the method above, the performance score dropped to
              0.237.
            </p>
            <p>
              While analyzing the model outputs above, we made a few
              observations on cases where model does not perform well. Note that
              these are not statistical ways of finding edge cases but are more
              inspired by our intuition in dealing with the above data.
            </p>
            <h2 id="Catching-Edge-cases-for-Finetuning-the-Model-Later">
              Catching Edge-cases for Finetuning the Model Later
            </h2>
            <p>
              A basic step in fine-tuning the model is to analyze its
              performance and identify situations where it {"doesn't"} perform
              well to catch appropriate edge-cases. We do this by making a few
              observations on the model performance.
            </p>
            <p>
...
"Jen wants to break up with her boyfriend. He hasn't paid her back the"`}
            ...
            <p>
              ...
              To collect a dataset that can help the model improve in these edge
              cases, we need to find conversations that are longer than a
              certain threshold. To find an appropriate threshold, we generated
              a histogram of length of input dialogues on the training dataset
              (i.e., SAMSum train). From here, we noted that a length of 1700
              can be a good cut-off to collect large conversation data-points.
...
edge_case_length = {
    ...
    'signal_formulae': uptrain.Signal("Length_dialog", length_check_func)
            ...
            <p>
              <b>Observation</b>: Next, we will discuss another type of edge
              case that affects the performance of our language model. In this
              case, the model directly copies one or two sentences from the
              input conversation. This works well in many cases but fails when
              the conversation is about refuting those one or two sentences
              which model produced as summary. This can lead to the generation
              of summaries that are not accurate or do not capture the true
              essence of the conversation.
            </p>
            <p>
              For example, the output summary of the model in the following
...
Output: Person1 is looking for a shop that sells inexpensive cashmere sweaters.`}</CodeViewer>
            ...
            <p>
              ...
              calculates the longest common subsequence of characters in the
            ...
            <p>
              ...
              check the vocabulary coverage of the new dataset (DialogSum) on
              the old dataset (SAMSum). By evaluating the vocabulary coverage,
              we can determine whether there is a significant shift in the
              vocabulary used between the two datasets. This will help us
              understand how well the model can generalize and produce accurate
              summaries for different datasets.
            </p>
            <p>
    ...
    'need_gt': False,
            ...
            <p>
              By using this custom monitor, we can analyze the vocabulary
              coverage of our model and identify any vocabulary drift that might
              be affecting the {"model's"} performance. This allows us to make
              the necessary adjustments to improve the accuracy and reliability
              of our language model in generating summaries for various
              datasets.
            </p>
            <p>
            ...
            <p>
              ...
              and custom monitor we defined earlier. Additionally, we specify
              the folder where the smart data will be stored.
            </p>
            <CodeViewer>
              {`config = {
...
framework = uptrain.Framework(cfg_dict=config)`}
            ...
            <p>
              ...
              coverage of our model on the production data. The coverage starts
              at around 98% for the SAMSum test dataset but decreases to about
              95% for the DialogSum dataset (that is, after ~800 SAMSum test
              points are logged).
            </p>
            <Image
              style={{ height: "auto" }}
            ...
            />
            <p>
              By inspecting the collected edge cases, we can confirm that our
              edge case detector is effectively identifying the appropriate
              cases and collects 554 edge-cases of the 13200 data-points logged
              into the framework.
            </p>
            <p>
            ...
            <p>
              {"We've"} found that many conversations in the DialogSum dataset
              are focused on the Asia region which were not originally present
              in the SAMSum dataset. So, {"we'll"} create a check to catch cases
              containing specific Asian words. The list asian_words contains
              some common Asian words like names, places, and currencies, and
              the check goes on to define cases where Asia-related words are
              found in the input dialogue.
            </p>
            <CodeViewer>
...
edge_case_asian_word = {
    ...
    'signal_formulae': uptrain.Signal("asian_word", asian_words_check)
            ...
            <p>
              ...
              specific weaknesses in the {"model's"} performance. This dataset
              can then be used to fine-tune the model, making it better at
